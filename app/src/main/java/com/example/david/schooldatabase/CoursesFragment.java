...
public class CoursesFragment extends Fragment implements LoaderCallbacks<Cursor> {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (intent != null && intent.hasExtra(Intent.EXTRA_TEXT)) {
            mCourseStr = intent.getStringExtra(Intent.EXTRA_TEXT);
            //((TextView) rootView.findViewById(R.id.txtIndexNo)).setText(mCourseStr);
        }

        loadermanager=getLoaderManager();
        mListView = (ListView) rootView.findViewById(R.id.courseListView);

        mAdapter = new SimpleCursorAdapter(getActivity(),
                R.layout.list_course_records,
                null,
                new String[] {SchoolDBProvider._ID,SchoolDBProvider.COURSE_CODE, SchoolDBProvider.COURSE_TITLE, SchoolDBProvider.CREDITS},
                new int[] { R.id.txtId, R.id.txtCourseCode , R.id.txtCourseTitle, R.id.txtCredits}, 0);
    ...
    public Loader<Cursor> onCreateLoader(int arg0, Bundle arg1) {
        ...
        return new CursorLoader(getActivity(), uri, courseData, null, null, null);
    }
